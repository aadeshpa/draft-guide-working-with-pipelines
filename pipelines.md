...
- [dockerimage-name-lowercase-task.yaml](https://github.com/kabanero-io/kabanero-pipelines/blob/master/pipelines/incubator/dockerimage-name-lowercase-task.yaml)
   ...
   This task is needed since build and push of a container image requires its imagename to be lowercased as per the docker requirements. This task takes docker `docker-imagename` and docker `docker-imagetag` as input params and it builds the `docker-image` resource URL path with `docker-imagename` as lowercase letters. In the case when the input param `docker-imagename` is empty or not provided it will construct the `docker-image` URL path by using the imagename from existing `app-deploy.yaml` file. This `app-deploy.yaml` the file should be present in the `git-source` project provided as an input to the pipeline.
